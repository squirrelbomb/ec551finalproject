...
int zed_hdmi_display_cbars( zed_hdmi_display_t *pDemo, Xuint32 offset )
{
   ...
   volatile Xuint32 *pStorageMem = (Xuint32 *)pDemo->uBaseAddr_MEM_HdmiDisplay;
     ...
     FRESULT Res;
        	...
        	Res=f_close(&fil);
        	int count1,count2,countrow,countCOL,a,b,i,j,m=0,n=0,pixel_count=0;
   for ( frame = 0; frame < pDemo->uNumFrames_HdmiDisplay; frame++ )
   {
     /* for ( row = 0; row < pDemo->hdmio_height; row++ )
      {
         for ( col = 0; col < pDemo->hdmio_width; col++ )
         {
          ...
          if(col<384 && row<216) *pStorageMem++ = 0x00000000;
          else if(col<768 && col>=384 && row<432 && row>=216) *pStorageMem++ = 0x00FF0000;
          else if(col<1152 && col>=768 && row<648 && row>=432) *pStorageMem++ = 0x0000FF00;
          else if(col<1536 && col>=1152  && row<864 && row>=648) *pStorageMem++ = 0x000000FF;
          else if(col<=1920 && col>=1536 && row<=1080 && row>=864) *pStorageMem++ = 0x0000FFFF;
          else *pStorageMem++ = 0x00FFFF00;
         ...
         //   case 7: pixel = 0x00FFFFFF; break; // White
            ...
            //*pStorageMem++ = pixel;
					...
					for(countrow=0;countrow<100;countrow++){
						for(j=0;j<10;j++){

								for(countCOL=0;countCOL<100;countCOL++){

									  for(i=0;i<10;i++){

										  if(buffer[100*n+m]=='0'){*pStorageMem++ = 0x00000000;pixel_count++;}//black
										  else if(buffer[100*n+m]=='2'){*pStorageMem++ = 0x000000FF;pixel_count++;}//green
										  else if(buffer[100*n+m]=='1'){*pStorageMem++ = 0x00FF0000;pixel_count++;}//red
